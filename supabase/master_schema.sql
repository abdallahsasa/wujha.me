...
DECLARE _result jsonb; _ticket record; _inserted_ids jsonb := '[]'::jsonb;
BEGIN
  FOR _ticket IN SELECT * FROM jsonb_array_elements(_tickets) LOOP
    INSERT INTO public.tickets (
      ...
      qr_code, status, payment_status, payment_method, payment_reference, payment_amount
    ) VALUES (
      (_ticket.value->>'event_id')::uuid, (_ticket.value->>'ticket_type_id')::uuid,
      CASE WHEN _ticket.value->>'allocation_id' IS NOT NULL THEN (_ticket.value->>'allocation_id')::uuid ELSE NULL END,
      CASE WHEN _ticket.value->>'user_id' IS NOT NULL THEN (_ticket.value->>'user_id')::uuid ELSE NULL END,
      _ticket.value->>'guest_name', _ticket.value->>'guest_phone', _ticket.value->>'guest_email',
      _ticket.value->>'qr_code', _ticket.value->>'status', _ticket.value->>'payment_status',
      _ticket.value->>'payment_method', _ticket.value->>'payment_reference',
    ...
    -- If allocation_id was provided, increment usage
    IF (_ticket.value->>'allocation_id') IS NOT NULL THEN
      UPDATE public.sub_organizer_allocations 
      SET used_count = used_count + 1 
      WHERE id = (_ticket.value->>'allocation_id')::uuid;
    END IF;
